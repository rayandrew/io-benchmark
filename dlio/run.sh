...
jobid=$(flux --parent batch \
  ...
  --env=PPN="${PPN}" \
  --env=APP_ID="${APP_ID}" \
  --env=DLIO_WORKLOAD="${DLIO_WORKLOAD}" \
  --env=DLIO_GENERATE_DATA="${DLIO_GENERATE_DATA}" \
  --env=DLIO_TRAIN="${DLIO_TRAIN}" \
  --env=DLIO_CHECKPOINT="${DLIO_CHECKPOINT}" \
  --env=DLIO_EVAL="${DLIO_EVAL}" \
