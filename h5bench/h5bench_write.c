...
make_compound_type()
{
    DFTRACER_C_REGION_START(make_compound_type);
    ...
    DFTRACER_C_REGION_END(make_compound_type);
...
{
    DFTRACER_C_REGION_START(make_compound_type_separates);
    ...
    DFTRACER_C_REGION_END(make_compound_type_separates);
...
{
    DFTRACER_C_REGION_START(prepare_data_interleaved);
    ...
    *data_size_out = particle_cnt * sizeof(particle);
    DFTRACER_C_REGION_END(prepare_data_interleaved);
    return data_out;
}
...
{
    DFTRACER_C_REGION_START(prepare_data_contig_1D);
    ...
    *data_size_out = particle_cnt * (7 * sizeof(float) + sizeof(int));
    DFTRACER_C_REGION_END(prepare_data_contig_1D);
    return data_out;
}
...
prepare_data_contig_2D(unsigned long long particle_cnt, long dim_1, long dim_2, unsigned long *data_size_out)
{
    ...
    DFTRACER_C_REGION_START(prepare_data_contig_2D);
    ...
    *data_size_out = particle_cnt * (7 * sizeof(float) + sizeof(int));
    DFTRACER_C_REGION_END(prepare_data_contig_2D);
    return data_out;
}
                       ...
                       unsigned long *data_size_out)
{
    ...
    DFTRACER_C_REGION_START(prepare_data_contig_3D);
    ...
    *data_size_out = particle_cnt * (7 * sizeof(float) + sizeof(int));
    DFTRACER_C_REGION_END(prepare_data_contig_3D);
    return data_out;
}
...
{
    DFTRACER_C_REGION_START(data_free);
    assert(data);
    switch (mode) {
    ...
    DFTRACER_C_REGION_END(data_free);
}

void
set_dspace_plist(hid_t *plist_id_out, int data_collective)
{
    DFTRACER_C_REGION_START(set_dspace_plist);
    *plist_id_out = H5Pcreate(H5P_DATASET_XFER);
    ...
    DFTRACER_C_REGION_END(set_dspace_plist);
}

int
set_select_spaces_default(hid_t *filespace_out, hid_t *memspace_out)
{
    DFTRACER_C_REGION_START(set_select_spaces_default);
    hsize_t count[1] = {1};
    ...
    *memspace_out    = H5Screate_simple(1, (hsize_t *)&NUM_PARTICLES, NULL);
    ...
    DFTRACER_C_REGION_END(set_select_spaces_default);
    return 0;
}
...
set_select_spaces_strided(bench_params params, hid_t *filespace_out, hid_t *memspace_out)
{
    ...
    DFTRACER_C_REGION_START(set_select_spaces_strided);
    ...
    *filespace_out = H5Screate_simple(1, (hsize_t *)&TOTAL_PARTICLES, NULL);
    ...
    DFTRACER_C_REGION_END(set_select_spaces_strided);
...
{ 
    DFTRACER_C_REGION_START(set_select_space_2D_array);
    ...
    *memspace_out  = H5Screate_simple(2, mem_dims, NULL);
    ...
    DFTRACER_C_REGION_END(set_select_space_2D_array);
    return 0;
}
...
{
    DFTRACER_C_REGION_START(set_select_space_multi_3D_array);
    hsize_t mem_dims[3];
    hsize_t file_dims[3];
    mem_dims[0]  = (hsize_t)dim_1;
    mem_dims[1]  = (hsize_t)dim_2;
    ...
    *memspace_out  = H5Screate_simple(3, mem_dims, NULL);
    ...
    DFTRACER_C_REGION_END(set_select_space_multi_3D_array);
    return 0;
}
...
{
    DFTRACER_C_REGION_START(data_write_contig_contig_MD_array);
    ...
    *data_time     = t3 - t2;
    ...
    DFTRACER_C_REGION_END(data_write_contig_to_interleaved);
}
...
data_write_contig_to_interleaved(time_step *ts, hid_t loc, hid_t *dset_ids, hid_t filespace, hid_t memspace,
                                 ...
                                 unsigned long *data_time)
{
    DFTRACER_C_REGION_START(data_write_contig_to_interleaved);
    assert(data_in && data_in->x);
    hid_t dcpl;
    if (COMPRESS_INFO.USE_COMPRESS)
        dcpl = COMPRESS_INFO.dcpl_id;
...
data_write_interleaved_to_contig(time_step *ts, hid_t loc, hid_t *dset_ids, hid_t filespace, hid_t memspace,
                                 ...
                                 unsigned long *data_time)
{
    DFTRACER_C_REGION_START(data_write_interleaved_to_contig);
    assert(data_in);
    hid_t dcpl;
    if (COMPRESS_INFO.USE_COMPRESS)
        dcpl = COMPRESS_INFO.dcpl_id;
    ...
    *data_time     = t3 - t2;
    if (MY_RANK == 0)
        printf("    %s: Finished writing time step \n", __func__);
    DFTRACER_C_REGION_END(data_write_interleaved_to_contig);
}
...
{
    DFTRACER_C_REGION_START(data_write_interleaved_to_interleaved);
    assert(data_in);
    hid_t dcpl;
    if (COMPRESS_INFO.USE_COMPRESS)
        dcpl = COMPRESS_INFO.dcpl_id;
    ...
    *data_time     = t3 - t2;
    if (MY_RANK == 0)
        printf("    %s: Finished writing time step \n", __func__);
    DFTRACER_C_REGION_END(data_write_interleaved_to_interleaved);
}
...
{
    DFTRACER_C_REGION_START(_prepare_data);
    void *data = NULL;
    ...
    *data_preparation_time = get_time_usec() - t_prep_start;
    DFTRACER_C_REGION_END(_prepare_data);
    return data;
}
...
{
    DFTRACER_C_REGION_START(_run_benchmark_write);
    ...
    for (int ts_index = 0; ts_index < timestep_cnt; ts_index++) {
        ...
        if (ts_index != timestep_cnt - 1) { // no sleep after the last ts
            if (params.compute_time.time_num >= 0) {
                if (MY_RANK == 0)
                    printf("Computing...\n");
                DFTRACER_C_REGION_START(compute);
                async_sleep(ts->es_data, params.compute_time);
                DFTRACER_C_REGION_END(compute);
            }
        }
    ...
    *total_data_size_out = timestep_cnt * data_size;
    ...
    H5Sclose(filespace);
    H5Pclose(plist_id);
    DFTRACER_C_REGION_END(_run_benchmark_write);
    return 0;
}
...
{
    DFTRACER_C_REGION_START(set_globals);
    ...
    DFTRACER_C_REGION_END(set_globals);
}

hid_t
set_fapl()
{
    DFTRACER_C_REGION_START(set_fapl);
    hid_t fapl = H5Pcreate(H5P_FILE_ACCESS);
    DFTRACER_C_REGION_END(set_fapl);
    return fapl;
}
...
{
    DFTRACER_C_REGION_START(set_metadata);
    ...
    DFTRACER_C_REGION_END(set_metadata);
    return fapl;
}
